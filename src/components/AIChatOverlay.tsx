...
export function AIChatOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function handleSend() {
    ...
    // KHOEM-AI brain: a deterministic interpreter + safety firewall
    // (src/core/ai/khoem-ai-brain.ts). It classifies intent and returns a
    // verified, safe message — it never bypasses Authentication,
    // Authorization, or the Safety Engine. No external API key is used.
