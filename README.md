...
# ច្បាប់ Project — KSV (ត្រកូល ខឹម ណាវ)

សូម AI ណាមួយក៏ដោយ (Bolt, Claude, ChatGPT, ឬឧបករណ៍ផ្សេងទៀត) ដែលធ្វើការលើ
project នេះ **អាន និងអនុវត្តតាមច្បាប់ខាងក្រោមនេះយ៉ាងតឹងរឹង** មិនមានករណីលើកលែង។

## ១. ថត Project Root មានតែមួយ

```
KSV/
└── khoem-now/     ← នេះជា Project Root តែមួយគត់
```

- ឈ្មោះថត `khoem-now` តំណាងឲ្យ **ត្រកូល ខឹម ណាវ** ជាម្ចាស់ project នេះ។
- **ហាមបង្កើតថត Root ថ្មីទៀត** មិនថាឈ្មោះអ្វី (មិនត្រូវបង្កើត `khoem-now-v2`,
  `khoem-now-new`, `khoem-now-backup`, `project2` ។ល។)
- កូដ file ថ្មីទាំងអស់ ត្រូវដាក់ចូល **ក្នុង `khoem-now/`** ប៉ុណ្ណោះ
  (អាចបង្កើត subfolder ក្នុងនោះបាន ដូចជា `src/`, `api/` ។ល។)

## ២. ហាមបង្កើត Backup ដោយខ្លួនឯងក្នុង Repo

- **កុំ copy ថតទាំងមូលធ្វើ backup** ក្នុង project (ដូចជា `backup-before-fix/`,
  `archive/`, `khoem-now-backup-20260828/`) ។
- ប្រវត្តិការផ្លាស់ប្តូរទាំងអស់ ត្រូវប្រើ **Git commit history** ជា backup
  ជំនួសវិញ (`git commit`, `git log`) — នេះជាមធ្យោបាយត្រឹមត្រូវរបស់ Git ខ្លួនឯង
  ដើម្បីតាមដានកំណែចាស់ ដោយមិនចាំបាច់ចម្លងថតទាំងមូល។
- បើត្រូវការសាកល្បងផ្លាស់ប្តូរដ៏ធំ ប្រើ **Git branch** ជំនួសការចម្លងថត។

## ៣. ថត `node_modules` មិនត្រូវ Commit ចូល GitHub

- `node_modules` ត្រូវនៅក្នុង `.gitignore` ជានិច្ច
- Library ទាំងអស់ត្រូវទាញតាម `npm install` ពី `package.json` វិញ
  មិនត្រូវរក្សាទុកលើ GitHub ទេ (ធ្វើឲ្យ repo ធ្ងន់ និងយឺត)

## ៤. មុនបង្កើត File ឬ Folder ថ្មី

AI ត្រូវ **ពិនិត្យសិនថាមានស្រាប់ហើយឬអត់** នៅក្នុង `khoem-now/`៖
- បើមានស្រាប់ → **កែប្រែ (edit)** file/folder ដដែល
- បើមិនទាន់មាន → បង្កើតថ្មី **ក្នុង `khoem-now/`** តាមរចនាសម្ព័ន្ធ Domain
  ដែលបានកំណត់ស្រាប់ (`src/i18n/`, `src/data/`, `src/views/`,
  `src/components/` ។ល។)

## ៥. គោលការណ៍ចុងក្រោយ

> **កន្លែងតែមួយ សម្រាប់ការពិតតែមួយ។**
> មិនមានច្បាប់ចម្លងទីពីរសម្រាប់ file ឬ folder ណាមួយឡើយ។

ប្រសិនបើ AI ណាមួយមិនច្បាស់ ត្រូវសួរម្ចាស់ project (ខឹម ណាវ)ជាមុនសិន
មុននឹងបង្កើត folder ថ្មី ឬចម្លង file ជាច្រើនកន្លែង។
